...
function openGift() {
  ...
  requestAnimationFrame(() => {
    box.style.animation =
      "gift-shake 720ms cubic-bezier(0.36, 0.07, 0.19, 0.97) both";

    setTimeout(() => {
      // Fly lid
      lid.style.animation =
        "lid-fly 1.05s cubic-bezier(0.22, 1, 0.36, 1) forwards";

      // Bloom flowers burst
      setTimeout(() => burstFlowers(), 320);

      // Start petal rain
      setTimeout(() => startPetalRain(), 650);

      // Fade to main site
      setTimeout(() => transitionToMain(), 3300);
    }, 560);
  });
}
...
function burstFlowers() {
  ...
  BLOOM_FLOWERS.forEach((flower, i) => {
    setTimeout(() => {
      ...
      el.style.animation =
        "bloom-burst 2.2s cubic-bezier(0.25,0.46,0.45,0.94) forwards";
...
function startPetalRain() {
  ...
  const interval = setInterval(() => {
    ...
    p.textContent =
      PETAL_SYMBOLS[Math.floor(Math.random() * PETAL_SYMBOLS.length)];
