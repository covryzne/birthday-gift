/* =============================================================
   ...
   =============================================================
  Place music at:  assets/audio/song1.mp3   etc.
...
const AudioEngine = (() => {
  ...
  function cinematicFade(dur = 2) {
    if (isPlaying) fadeTo(0, dur);
  }
  function cinematicFadeIn(dur = 2) {
    if (isPlaying && currentConfig) fadeTo(currentConfig.volume, dur);
  }
  ...
  function getProgress() {
    if (!isPlaying || !currentConfig || !audioCtx)
      return { current: 0, duration: 0, pct: 0 };
    const start = currentConfig.startTime || 0;
    const end =
      currentConfig.endTime || (currentBuffer ? currentBuffer.duration : 0);
...
const UI = (() => {
  ...
  function startVizLoop() {
    ...
    const W = vizCanvas.width,
      H = vizCanvas.height;
    const bars = 20,
      step = Math.floor(data.length / bars);
